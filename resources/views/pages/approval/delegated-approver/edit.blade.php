@section('content')
<form method="POST" action="{{ route('approval.delegated-approver.update', $data->id) }}">
@csrf
@method('PUT')
    <div class="modal-body">
        ...
        <div class="form-group">
            <select class="form-control select2-format" name="workflow_approver_id" required>
                ...
                @foreach($workflowApprovers as $approver)
                    <option value="{{ $approver->id }}" {{ old('workflow_approver_id', $data->workflow_approver_id) == $approver->id ? 'selected' : '' }}>
        ...
        <div class="form-group">
            <select class="form-control select2-format" name="delegate_user_id" required>
                ...
                @foreach($users as $user)
                    <option value="{{ $user->id }}" {{ old('delegate_user_id', $data->delegate_user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                @endforeach
            </select>
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="form-group">
                    <input type="date" class="form-control" name="start_date" required value="{{ old('start_date', $data->start_date?->format('Y-m-d')) }}">
            ...
            <div class="col-md-6">
                ...
                <div class="form-group">
                    <input type="date" class="form-control" name="end_date" required value="{{ old('end_date', $data->end_date?->format('Y-m-d')) }}">
