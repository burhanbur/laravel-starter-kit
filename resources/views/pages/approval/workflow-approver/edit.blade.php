@section('content')
<form method="POST" action="{{ route('approval.workflow-approver.update', $data->id) }}">
@csrf
@method('PUT')
    <div class="modal-body">
        ...
        <div class="form-group">
            <select class="form-control select2-format" name="workflow_approval_stage_id" required>
                ...
                @foreach($stages as $stage)
                    <option value="{{ $stage->id }}" {{ old('workflow_approval_stage_id', $data->workflow_approval_stage_id) == $stage->id ? 'selected' : '' }}>
        ...
        <div class="form-group">
            <select class="form-control select2-format" name="approval_type_id" required>
                ...
                @foreach($approverTypes as $type)
                    <option value="{{ $type->id }}" {{ old('approval_type_id', $data->approval_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                @endforeach
            </select>
        ...
        <label>Catatan</label>
        <div class="form-group">
            <textarea class="form-control" name="remarks" rows="2" placeholder="Catatan (opsional)">{{ old('remarks', $data->remarks) }}</textarea>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="is_optional" value="1" id="is_optional" {{ old('is_optional', $data->is_optional) ? 'checked' : '' }}>
                    <label class="form-check-label" for="is_optional">Persetujuan Opsional</label>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="can_delegate" value="1" id="can_delegate" {{ old('can_delegate', $data->can_delegate) ? 'checked' : '' }}>
