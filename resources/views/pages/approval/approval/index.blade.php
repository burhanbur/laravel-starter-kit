...
@section('title', config('app.alias') . ' | Aksi Approval')
            ...
            <div class="kt-portlet__head">
                <div class="kt-portlet__head-label">
                    <h3 class="kt-portlet__head-title">
                        <i class="flaticon2-check-mark"></i> &nbsp; Aksi Approval
                    </h3>
                </div>
                <div class="kt-portlet__head-toolbar">
                    <div class="kt-portlet__head-wrapper">
                        <div class="kt-portlet__head-actions">
                            <a href="javascript:void(0)" value="{{ route('approval.approval.create') }}" class="btn btn-icon btn-success modalInterval" data-bs-toggle="modal" title="Tambah Aksi Approval" data-bs-target="#modalInterval"><i class="la la-plus"></i></a>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                <td class="text-center align-middle">
                                    @if($item->approvalStatus)
                                        <span class="badge badge-info">{{ $item->approvalStatus->name }}</span>
                                    @else
                                ...
                                <td class="align-middle d-none d-md-table-cell">{{ $item->approved_at ? $item->approved_at->format('d/m/Y H:i') : '-' }}</td>
                                <td class="text-center align-middle">
                                    <span class="tooltips" data-original-title="Detail">
                                        <a href="{{ route('approval.approval.show', $item->id) }}" class="btn btn-sm btn-icon btn-primary"><i class="fa fa-eye"></i></a>
                                    </span>
                                </td>
                            </tr>
                            @endforeach
