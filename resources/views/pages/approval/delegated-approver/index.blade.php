...
@section('title', config('app.alias') . ' | Delegasi Approver')
            ...
            <div class="kt-portlet__head">
                <div class="kt-portlet__head-label">
                    <h3 class="kt-portlet__head-title">
                        <i class="flaticon2-avatar"></i> &nbsp; Delegasi Approver
                    </h3>
                </div>
                <div class="kt-portlet__head-toolbar">
                    <div class="kt-portlet__head-wrapper">
                        <div class="kt-portlet__head-actions">
                            <a href="javascript:void(0)" value="{{ route('approval.delegated-approver.create') }}" class="btn btn-icon btn-success modalInterval" data-bs-toggle="modal" title="Tambah Delegasi Approver" data-bs-target="#modalInterval"><i class="la la-plus"></i></a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <table class="table table-striped table-hover table-bordered" id="myDataTables" style="width: 100%;">
                        <thead>
                            <tr>
                                ...
                                <td class="text-center">Workflow Approver</td>
                                <td class="text-center">Delegasi Kepada</td>
                                <td class="text-center">Tanggal Mulai</td>
                                <td class="text-center">Tanggal Selesai</td>
                                <td class="text-center d-none d-md-table-cell">Status Aktif</td>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="align-middle">{{ $item->workflowApprover->stage->name ?? '-' }}</td>
                                <td class="align-middle">{{ $item->delegateUser->name ?? '-' }}</td>
                                <td class="align-middle">{{ $item->start_date ? $item->start_date->format('d/m/Y') : '-' }}</td>
                                <td class="align-middle">{{ $item->end_date ? $item->end_date->format('d/m/Y') : '-' }}</td>
                                ...
                                <td class="text-center align-middle">
                                    <form action="{{ route('approval.delegated-approver.destroy', $item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <span class="tooltips" data-original-title="Ubah">
                                            <a href="javascript:void(0)" value="{{ route('approval.delegated-approver.edit', $item->id) }}" class="btn btn-sm btn-icon btn-info modalInterval" data-bs-toggle="modal" title="Ubah Delegasi Approver" data-bs-target="#modalInterval"><i class="fa fa-edit"></i></a>
                                        </span>
