@section('content')
<form method="POST" action="{{ route('approval.approval-status.update', $data->id) }}">
@csrf
@method('PUT')
    <div class="modal-body">
        ...
        <div class="form-group">
            <input type="text" class="form-control" name="code" required value="{{ old('code', $data->code) }}" placeholder="Contoh: APPROVED">
            @error('code') <small class="text-danger">{{ $message }}</small> @enderror
        </div>

        <label>Nama <b class="text-danger">*</b></label>
        <div class="form-group">
            <input type="text" class="form-control" name="name" required value="{{ old('name', $data->name) }}" placeholder="Nama status">
